...
def searchData(MTy="", Ref="",Tit="",fna="",sna="", Adr1="", Adr2="", pcd="",MNo="", BkID="", Bkt="", Atr="",DBo="", Ddu="", sPr="",LrF="", DoD="", DonL=""):
	con=sqlite3.connect("libbooks.db")
	cur = con.cursor()
	cur.execute("SELECT * FROM libbooks WHERE MTY=? OR REF=? OR TIT=? OR FNA=? OR SNA=? \
		OR ADR1=? OR ADR2=? OR PCD=? OR MNO=? OR BKID=? OR BKT=? OR ATR=? \
		OR DBO=? OR DDU=? OR SPR=? OR LRF=? OR DOD=? OR DONL=?",(MTy, Ref,Tit,fna,sna, Adr1, Adr2, pcd,MNo, BkID, Bkt, Atr,DBo, Ddu, sPr,LrF, DoD, DonL))
	rows=cur.fetchall()
	con.close
	return rows
